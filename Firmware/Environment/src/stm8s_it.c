...
INTERRUPT_HANDLER(UART1_TX_IRQHandler, 17)
{
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */
  extern volatile uint8_t TurretData[TURR_MAX_PKT_LEN];
  extern volatile uint8_t TurretTxPointer;

  UART1_SendData8(TurretData[TurretTxPointer++]);

  if(TurretTxPointer >= TURR_MAX_PKT_LEN)
  {
